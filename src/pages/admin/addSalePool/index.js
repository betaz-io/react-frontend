...
const AddSalePool = () => {
  ...
  const handleUpdate = async () => {
    ...
    if (!hasRole?.toHuman().Ok) {
      ...
      if (poolInfo.toHuman()?.Ok) {
        toast.error("Pool exist!");
        setIsLoading(false);
        return;
      }
      ...
      await execContractTx(
        ...
        sale_pool_contract.CONTRACT_ADDRESS,
        0,
        "salePoolTrait::addPoolByPoolType",
        selected,
        status,
        endTime?.getTime(),
  ...
  return (
    <SectionContainer
      ...
      sx={{ marginTop: "100px" }}
    >
      <Text className="deposit-box-title">Add Sale Pool</Text>
